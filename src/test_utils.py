...
class TestTextToTextNodes(unittest.TestCase):
    ...
    def test_no_md_node(self):
        text = "This is a simple text block"
        new_nodes = text_to_textnodes(text)
        self.assertListEqual(
            [TextNode("This is a simple text block", TextType.TEXT)], new_nodes
        )

    
    def test_combined_block_some_invalid_md(self):
        text = "This is **text** with an _italic_ word and an `invalid code block and an ![obi wan image](https://i.imgur.com/fJRm4Vk.jpeg) and a [link](https://boot.dev)"
        with self.assertRaises(ValueError):
